...
# Import your existing core modules exactly as they were
...
def run():
    ...
    # 1. Use console.status to create an animated spinner while searching
    ...
    # 2. Build a beautiful Table for the search results
    ...
    # 3. Use IntPrompt to safely handle user input
    try:
        # We add +1 to the range because the user sees 1-based indexing, not 0-based
    ...
    if media['media_type'] == 'tv':
        ...
        if details:
            # Wrap the TV info in a clean bordered Panel
    ...
    # 4. Another spinner while the headless browser does its magic
    ...
    # Check if data exists AND if the stream was found inside it
    ...
    if args.download:
        ...
        # Note: yt-dlp needs different flags to embed subs, so we keep the download simple for now
        download(real_stream, title_display) 
    else:
        console.print(f"[bold green]Starting player for:[/bold green] {title_display}")
        play(real_stream, subtitles) # Pass the subs to the player!
...
# Import your existing cache module!
...
def run():
    parser = argparse.ArgumentParser(prog="movie")
    parser.add_argument("query", nargs="*", help="Movie or Show name") # Changed to * so it's optional
    parser.add_argument("-d", "--download", action="store_true", help="Download content")
    ...
    if args.watchlist:
        ...
        # Convert dictionary to a list we can index
        items_list = list(saved_items.values())
        ...
        try:
            valid_choices = [str(i) for i in range(len(items_list) + 1)]
            choice_idx = IntPrompt.ask("\nSelect a number to play (0 to exit)", choices=valid_choices) - 1
            if choice_idx < 0: return
            
            # Map the saved item back into our working variables
            media = items_list[choice_idx]
    ...
    # --- SEARCH MODE ---
        ...
        if action == "watchlist":
            ...
            # Create a unique ID so we don't save duplicates
        ...
        # If they chose download, override the args
        if action == "download":
            args.download = True

    else:
        # If they ran `python main.py` with no flags and no query
        parser.print_help()
        return

    # --- RESOLVE AND EXECUTE (Happens for both Search and Watchlist) ---
    stream_page = scraper.get_stream_url(media, season, episode)
